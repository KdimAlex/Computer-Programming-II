...
public class Heap <E> extends AbstractQueue<E> {
  ...
  */
  public E poll() {
    ...
    ///
    if (size() == 1){
      E thingBeingRemoved = theElements.get(0);
      theElements.remove(0);
      return thingBeingRemoved;
    }
...
//         comparisonBetweenChildren = compare(left(index), right(index));
//      }
//
//      if(comparisonBetweenChildren < 0){
//        index = swap(index, left(index));
//      } else if(comparisonBetweenChildren > 0){
//        index = swap(index, right(index));
//      }else {
//
//        if (isValid(right(index)) && compare(index, right(index)) > 0) {
//          index = swap(index, right(index));
//        } else if (isValid(left(index)) && compare(index, left(index)) < 0) {
//          index = swap(index, right(index));
//        }
//
//      }
//      if(compare(index, right(index)) < 0){
//        index = swap(index, right(index));
//      } else {
//        index = swap(index, left(index));
//      }
...
//    int comparisonResult = compare(left(index), right(index));
//    if (comparisonResult < 0){
//      while (compare(index, left(index)) < 0){
//
//      }
//    } else {
//
...
//      return false;
    ...
    ///
    E lastElement = theElements.remove(size()-1);
    theElements.set(index,lastElement);
    ///
