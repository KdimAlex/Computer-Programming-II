...
public class TestLinkedMap extends LinkedMap {
	...
	boolean testRemove (Entry[] in, String s, Entry[] out) {
		set(in);
		try {
			Entry pair = find(s);
			remove(pair);
	...
	void testRemove() {
		{
			// Test removing the only entry in the map
			Entry[] in = {a1};
			Entry[] out = {};
			check(testRemove(in, "a", out));
		}
		{
			// Test removing the first entry in the map
			Entry[] in = {a1, b2, c3};
			Entry[] out = {b2, c3};
			check(testRemove(in, "a", out));
		}
		{
			// Test removing the last entry in the map
			Entry[] in = {a1, b2, c3};
			Entry[] out = {a1, b2};
			check(testRemove(in, "c", out));
		}
		{
			// Test removing a middle entry in the map
			Entry[] in = {a1, b2, c3, d4};
			Entry[] out = {a1, c3, d4};
			check(testRemove(in, "b", out));
		}
		{
			// Test removing a non-existent entry in the map
			Entry[] in = {a1, b2, c3};
			Entry[] out = {a1, b2, c3};
			check(testRemove(in, "d", out));
		}
	}
