...
//	    return (size != count); // fix
	...
	public E next () {
	    ...
	    E returnValue = theElements[(first + count) % theElements.length]; // fix
		count++;
	    // EXERCISE
	...
	for (E elem : theElements){
		newElements[n] = theElements[(first + n) % theElements.length];
		n++;
	}
	// EXERCISE
